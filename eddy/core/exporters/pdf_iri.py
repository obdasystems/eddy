...
class PdfIRIDiagramExporter(AbstractDiagramExporter):
    """
    ...
    def FillTableWithMetaDataInfoForRolesAndAttributes(self):
        ...
        wanted_attributes = ['functional', 'inverseFunctional', 'reflexive', 'irreflexive', 'symmetric', 'asymmetric',
                             'transitive']


        attributeNodes = self.project.iriOccurrences(item=Item.AttributeIRINode)
        attributeIris = [x.iri for x in attributeNodes]
        for attributeIRI in sorted(attributeIris,key=str):
            metaDict = attributeIRI.getMetaProperties()
            attributes = []
            ...
            attribute_predicate_plus_attributes.append(str(attributeIRI))
        ...
        roleNodes = self.project.iriOccurrences(item=Item.RoleIRINode)
        roleIris = [x.iri for x in roleNodes]
        for roleIRI in sorted(roleIris,key=str):
            metaDict = roleIRI.getMetaProperties()
            attributes = []
            ...
            role_predicate_plus_attributes.append(str(roleIRI))
    ...
    def FillTableWithIRIPrefixNodesDictionaryKeysAndValues(self):
        ...
        self.table.setItem(self.table.rowCount() - 1, 1, header_iri)
        self.table.setItem(self.table.rowCount() - 1, 0, header_prefix)
        self.table.setRowCount(self.table.rowCount() + 1)

        for prefix,ns in self.project.prefixDictItems():
            self.append_row_and_column_to_table(prefix, ns, None)
    ...
    def split_table_if_necessary_and_render_it(self, table, printer, painter, meta_tada_table=False):
        ...
        for t in tables:
            ...
            valid = printer.setPageSize(
                QtGui.QPageSize(QtCore.QSizeF(width_to_set + 3, height_to_set + 3), QtGui.QPageSize.Point))
            if not valid:
