...
class IunExplorerPlugin(AbstractPlugin):
    """
    This plugin provides the IUN Explorer widget.
    """
    ...
    def onSessionReady(self):
        ...
        widget = self.widget('IUN_explorer')
        ...
        # FILL IN IUN EXPLORER WITH DATA
        connect(self.sgnFakeItemAdded, widget.doAddNode)

        list_of_nodes_in_project = list(self.project.nodes())

        print('\nself.project.unsatisfiable_classes - ',self.project.unsatisfiable_classes)

        #print('\nfor project_node in list_of_nodes_in_project:')

        for project_node in list_of_nodes_in_project:

            #print('     project_node-',project_node)
            #print('         project_node.name',project_node.name)
            if project_node.text() in self.project.unsatisfiable_classes:
                #print('--------------------------->>>>>>>>>>>>>> self.sgnFakeItemAdded.emit(project_node.diagram, project_node)')
                self.sgnFakeItemAdded.emit(project_node.diagram, project_node)
        disconnect(self.sgnFakeItemAdded, widget.doAddNode)

        #print('\nfor project_node in list_of_nodes_in_project:END')
    ...
    def dispose(self):
        ...
        menu.removeAction(self.widget('IUN_explorer_dock').toggleViewAction())

        # UNINSTALL THE PALETTE DOCK WIDGET
        self.debug('Uninstalling docking area widget')
        self.session.removeDockWidget(self.widget('IUN_explorer_dock'))
    ...
    def start(self):
        ...
        self.debug('Creating IUN explorer widget')
        widget = IUNExplorerWidget(self)
        widget.setObjectName('IUN_explorer')
        self.addWidget(widget)

        # CREATE DOCKING AREA WIDGET
        self.debug('Creating docking area widget')
        widget = DockWidget('IUN Explorer', QtGui.QIcon(':icons/18/ic_explore_black'), self.session)
        widget.setAllowedAreas(QtCore.Qt.LeftDockWidgetArea|QtCore.Qt.RightDockWidgetArea)
        widget.setObjectName('IUN_explorer_dock')
        widget.setWidget(self.widget('IUN_explorer'))
        self.addWidget(widget)

        # CREATE ENTRY IN VIEW MENU
        self.debug('Creating docking area widget toggle in "view" menu')
        menu = self.session.menu('view')
        menu.addAction(self.widget('IUN_explorer_dock').toggleViewAction())
        ...
        self.session.addDockWidget(QtCore.Qt.RightDockWidgetArea, self.widget('IUN_explorer_dock'))


class IUNExplorerWidget(QtWidgets.QWidget):
    """
    This class implements the IUN explorer used to list IUN predicates.
    """
    ...
    def __init__(self, plugin):
        """
        Initialize the IUN explorer widget.
        :type plugin: Session
        """
        ...
        self.ontoview = IUNExplorerView(self)
...
class IUNExplorerView(QtWidgets.QTreeView):
    """
    This class implements the IUN explorer tree view.
    """
    def __init__(self, widget):
        """
        Initialize the IUN explorer view.
        :type widget: IUNExplorerWidget
        """
    ...
    def session(self):
        """
        Returns the reference to the Session holding the IUNExplorer widget.
        :rtype: Session
        """
    ...
    def widget(self):
        """
        Returns the reference to the IUNExplorer widget.
        :rtype: IUNExplorerWidget
        """
        return self.parent()
