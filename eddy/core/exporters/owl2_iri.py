...
class OWLOntologyExporterWorker_v3(AbstractWorker):
    ...
    def getPropertyAssertion(self, node):
        ...
        for operand in [node.diagram.edge(i).other(node) for i in node.inputs]:
            # if not (operand.type() is Item.IndividualNode or operand.type() is Item.ConceptNode):
            if not operand.type() in [Item.IndividualNode, Item.ConceptNode, Item.RoleNode,
    ...
    def createClassAssertionAxiom(self, edge):
        ...
        if OWLAxiom.ClassAssertion in self.axiomsList:
            ind = None
            if not edge.source.type() is Item.IndividualNode:
    ...
    def createSameIndividualAxiom(self, edge):
        ...
        if OWLAxiom.SameIndividual in self.axiomsList:
            ...
            for node in [edge.source, edge.target]:
                if node.type() is Item.ConceptNode or node.type() is Item.AttributeNode or node.type() is Item.RoleNode:
