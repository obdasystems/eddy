    ...
    def updateTail(self):
        ...
        if self.edge.complete:
            ...
            if intersection:
                ...
                tail = intersection[1]
                p1 = tail + QPointF(sin(angle + M_PI / 3.0) * self.size, cos(angle + M_PI / 3.0) * self.size)
                p2 = tail + QPointF(sin(angle + M_PI - M_PI / 3.0) * self.size, cos(angle + M_PI - M_PI / 3.0) * self.size)
                self.tail = QPolygonF([tail, p1, p2])
        else:
            self.tail = None
